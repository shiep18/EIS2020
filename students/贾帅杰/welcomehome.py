...
def shoufig(xo, yo, zo, L, W, H, num):
    if num >= 0 and num <= 27:
        fname = "huawen" + str(num) + ".csv"
        f = open(fname, "r")
        offsetX = 4
        offsetZ = 0
...
def build(xo, yo, zo, L, W, H, M, fig):
    global j
    for i in ban:
        if i == M:
            M = 169 
...
#绿化修饰
    for x in range(10,20) :
        for z in range(30) :
            mc.setBlock(xo + x, yo, zo + z, M)
            if z%2 == 0 :
                mc.setBlock(xo + x, yo, zo + z, block.WOOL.id)
    for x in range(10) :
        for z in range(10,30) :
            mc.setBlock(xo + x, yo, zo + z, M)
            if z in [15,16]:
                mc.setBlock(xo + x, yo+1, zo + z, 3)
                mc.setBlock(xo + x, yo+2, zo + z, 37)
#照明（海晶灯）
    for x in range(2):
        for z in range(2):
...
for d in range(60):
    mc.setBlock(pos.x + d, pos.y + 60, pos.z + 73, 8)
