...
class mytimercallback():
   ...
   def execute(self,obj,event):
      ...
      if resp !="":
         ...
         self.cam=speeds[2]
         self.flag=1
         print("speed update",self.leftspeed,self.rightspeed,self.cam)
      else :
         self.flag=0
      self.actors[0].RotateZ(-(self.leftspeed//10));
      self.actors[1].RotateZ(-(self.rightspeed//10));
      if self.flag == 1:
         self.actors[2].RotateX(-self.t);
         self.actors[2].RotateX(self.cam);
         self.t=self.cam
      iren = obj
