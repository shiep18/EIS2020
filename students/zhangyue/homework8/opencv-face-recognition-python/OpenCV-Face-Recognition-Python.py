...
subjects = ["", "Zhang Yue", "Shao Qiong", "Yang Mi"]
...
while cap is not None:
    ...
    for x1, y1, w1, h1 in faces:
        cv2.imwrite("test-data/myHead.jpg", frame[y1 - 80:y1 + h1 + 60, x1 - 20:x1 + w1 + 20])
    cv2.imshow('frame', frame)
    myHead = cv2.imread("test-data/myHead.jpg")
    if myHead is not None:
        cv2.imshow('head', myHead)
        predicted_img1, name = predict_new(myHead)
        if name is not None:
            cv2.imshow(subjects[1], cv2.resize(predicted_img1, (400, 500)))
            print(name)
        os.remove("test-data/myHead.jpg")
        if name is 'Zhang Yue':
            mc.setBlocks(0, 0, 4, 0, 1, 5, 0)  # 开门
            run("1")
        else:
            mc.setBlocks(0, 0, 4, 0, 1, 5, 20)  # 关门
            run("0")
    cv2.waitKey(500)
cap.release()
cv2.destroyAllWindows()
