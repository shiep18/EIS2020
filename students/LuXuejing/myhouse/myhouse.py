...
def house(x0,y0,z0,L,W,H):
    ...
    for j in range(L):#地板
        for i in range(W):
            mc.setBlock(x0+i, y0, z0+j, 5)
    for j in range(H):#墙壁1
        for i in range(W):
            mc.setBlock(x0, y0+j, z0+i, block.GLASS.id)
            mc.setBlock(x0+W-1, y0+j, z0+i, block.GLASS.id)
    for j in range(H):#墙壁2
        for i in range(L):
            mc.setBlock(x0+i, y0+j, z0, block.GLASS.id)
            mc.setBlock(x0+i, y0+j, z0+L-1, block.GLASS.id)
    for j in range(3):#门
        for i in range(4):
            mc.setBlock(x0+L/3+j, y0+1+i, z0, 0)
    for j in range(2):#窗子
      for i in range(2): 
            mc.setBlock(x0+W-2, y0+i+2, z0+j+4, 20)
    for j in range(1):#火炬
        for i in range(2):
    ...
    for j in range(L):#使用随机数生成随机花纹
        for i in range(W):
            ...
            if num==0:
                ...
                mc.setBlock(x0+i, y0+H, z0+j, 31)
            elif num==2:
                mc.setBlock(x0+i, y0+H, z0+j, 38)
            elif num==3:
                mc.setBlock(x0+i, y0+H, z0+j, 19)
            elif num==4:
                mc.setBlock(x0+i, y0+H, z0+j, 18)
            elif num==5:
                mc.setBlock(x0+i, y0+H, z0+j, 40)
            elif num==6:
                mc.setBlock(x0+i, y0+H, z0+j, 24)
            elif num==7:
                mc.setBlock(x0+i, y0+H, z0+j, 4)
