...
#f=open("C:\\Apache\\htdocs\\index.html","r")
#data=f.read()
...
def setup(hass, config):
    ...
    _LOGGER.info("Get the configuration %s=%d",
                 CONF_STEP, step)
    f=open('C:\Apache24\htdocs\index0.html')
    ff=f.read()
...
class GrowingState(object):
    ...
    def update(self, now):
        ...
        #txt=open(r"C:\Users\F\AppData\Roaming\.homeassistant\custom_components\num.txt")
        #_LOGGER.info("GrowingState is updating"+txt.readline())
        f=open('C:\Apache24\htdocs\index0.html')
        ff=f.read()
        _LOGGER.info('Door is %s' %ff)
        # 状态值每次增加step
