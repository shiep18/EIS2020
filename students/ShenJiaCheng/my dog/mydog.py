...
def wakeUp(result,pinyin):
    if getPinYin("狗") in pinyin:
        if getPinYin("你好") in pinyin:
            ...
            #city=getCity(result)
            #print(city)
            temp = getWeatherInfo("上海")
            print(temp[0])
            mc.postToChat(rep(getPinYin(temp[1]))+":"+temp[2]+"C")
            pos = mc.player.getTilePos()
            mc.setBlocks(pos.x-2,pos.y,pos.z+8,pos.x+2,pos.y+30,pos.z+12,0)
            for i in range(eval(temp[2])):
                mc.setBlock(pos.x,pos.y+i,pos.z+10,35)
                if i%5==0:
                    mc.setBlock(pos.x,pos.y+i,pos.z+10,46)
        ...
        elif getPinYin("向左") in pinyin:
            move("left")
            
        elif getPinYin("向右") in pinyin:
            move("right")
            
        elif getPinYin("向前") in pinyin:
            move("front")
            
        elif getPinYin("向后") in pinyin:
            move("back")
