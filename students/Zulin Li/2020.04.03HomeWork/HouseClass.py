    ...
    def SetHouse(self):#(x,y,z)表示屋子的第一个方块所在地
        for X in range(0,self.l):
            for Z in range(0,self.w):
                mc.setBlock(self.x + X, self.y, self.z + Z, 98)# 底部
                mc.setBlock(self.x + X, self.y + self.h-1, self.z + Z, 98)#屋顶
        for Y in range(1, self.h-1):
            for X in range(0,self.l):
                mc.setBlock(self.x + X, self.y + Y, self.z,98)
                mc.setBlock(self.x + X, self.y + Y, self.z + self.w-1, 98)
            for Z in range(1, self.w-1):
                mc.setBlock(self.x,self.y + Y,self.z + Z, 98)
                mc.setBlock(self.x + self.l -1, self.y + Y, self.z + Z, 98)
        #门
        mc.setBlock(self.x + int(self.l / 2), self.y , self.z - 1, 44,[5])
        mc.setBlock(self.x + int(self.l / 2), self.y + 1, self.z,0)
        mc.setBlock(self.x + int(self.l / 2), self.y + 2, self.z, 0)
        mc.setBlock(self.x + int(self.l / 2) - 1, self.y , self.z - 1, 44,[5])
...
if __name__=='__main__':
    mc.player.setTilePos([506,0,500])
