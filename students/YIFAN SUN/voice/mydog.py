...
def showtem(t1,t2):
    ...
    reader1 = csv.reader(open(str(t1)+'.csv'))
    reader2 = csv.reader(open(str(t2)+'.csv'))
    data1=[]
    data2=[]
    for r in reader1:
        data1.append(r)
    for i in range(10):
        for j in range(10):
            if (data1[j][i]=='1'):
                mc.setBlock(pos.x+j,pos.y,pos.z+i+15,220)
    for r in reader2:
        data2.append(r)
    for i in range(10):
        for j in range(10):
            if (data2[j][i]=='1'):
                mc.setBlock(pos.x+j,pos.y,pos.z+i+25,220)
    ...
    if getPinYin("狗") in pinyin:
        if getPinYin("你好") in pinyin:
            ...
            for i in range(eval(temp[2])):
                mc.setBlock(pos.x+i-1,pos.y,pos.z+9,1)
                mc.setBlock(pos.x+i,pos.y,pos.z+10,233)
                mc.setBlock(pos.x+i-1,pos.y,pos.z+11,1)
            mc.setBlock(pos.x+i+1,pos.y,pos.z+10,1)
            for x in range(2):
                mc.setBlock(pos.x+i+x,pos.y,pos.z+11,1)
                mc.setBlock(pos.x+i+x,pos.y,pos.z+9,1)
