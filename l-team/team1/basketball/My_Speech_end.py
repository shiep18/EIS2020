...
# 打开数据库连接
db = pymysql.connect('localhost', 'root', '', 'basketball')
cursor = db.cursor()
...
def getBaiduText():
    ...
    stream.start_stream()
    num_basketball = sqlname_backnumber("wzy",1,10,'rest')
    #print(num_basketball)
...
def sqlname_backnumber(name,flag,number,need):  #flag=1 读取剩余球数/2:上传剩余球数/3：上传借球数
    if flag == 1:
        sql = "SELECT * FROM basketball WHERE name='"+name+"'"
    elif flag == 2:
        print(number)
        sql = "UPDATE `basketball` SET `rest`="+str(number)+" WHERE name='"+name+"'"
    elif flag == 3:
        sql = "UPDATE `basketball` SET `borrow_count`="+str(number)+" WHERE name='"+name+"'"
    try:
   # 执行sql语句
        print("ok")
        cursor.execute(sql)
        ...
        if flag == 1:
            ...
            for row in results:
                ...
                name = row[3]
                borrow_count = row[4]
                rest = row[6]  # 若有pic则为row 6
            # 打印结果
                #print ("grade=%s,class=%s,number=%s,name=%s,borrow_count=%s" %(grade, Class, number, name, borrow_count ))
                db.commit()
                if (need == 'rest'):
                    return rest
                elif(need == 'borrow'):
...
def wakeUp(result,pinyin):
    ...
    global Borrow
    num_basketball = int(sqlname_backnumber("wzy",1,10,'rest'))
    if (getPinYin("几个") in pinyin or getPinYin("多少") in pinyin or getPinYin("的数量")in pinyin ) and getPinYin("球") in pinyin :
    ...
    elif (getPinYin("需要")in pinyin or getPinYin("想要")in pinyin or getPinYin("要")in pinyin)and getPinYin("个篮球") in pinyin:
        
        num_basketball_else = str_num(result)
        if 0 < int(num_basketball_else) <= 10:
            ...
            #print("qwe",str(num_basketball))
            sqlname_backnumber('wzy',2,num_basketball,None)
            sqlname_backnumber('wzy',3,num_basketball_else,None) #借球数
        else:
        ...
        print("谢谢你的使用，请填写相关信息！")
        speaker.Speak("谢谢你的使用，请填写相关信息！")
        return 1
